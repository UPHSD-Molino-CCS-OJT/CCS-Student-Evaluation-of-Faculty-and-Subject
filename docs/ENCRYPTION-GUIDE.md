...
### Legacy Comments (Plaintext)

The system supports **both encrypted and plaintext comments** for backward compatibility:

- **Old evaluations**: Stored as plaintext strings (before encryption was enabled)
- **New evaluations**: Stored as encrypted objects (after ENCRYPTION_MASTER_KEY is configured)

The decryption function automatically handles both formats:
...
function decryptCommentsField(comments: any): string {
    ...
    // New: encrypted object
    if (typeof comments === 'object' && comments.encrypted) {
        return decryptField(comments); // Decrypt with master key
    }
...
### Privacy Audit Detection

The privacy audit (Layer 11) will detect mixed encryption states:

```
⚠ [Layer 11] Plaintext Comments Detected
  Found 15 evaluation(s) with unencrypted comments. 42 are encrypted.
  Recommendation: Legacy comments detected. New submissions are encrypted. 
                  Consider migration script for old data.
```

### Migration Script (Optional)

To encrypt legacy comments, create a migration script:

```typescript
// migrate-encrypt-comments.ts
import Evaluation from './models/Evaluation';
import { encryptField, isEncryptionConfigured } from './utils/encryption';
...
async function migrateComments() {
    ...
    const plaintextEvals = await Evaluation.find({
        comments: { $type: 'string', $ne: '' }
    });
    
    for (const evaluation of plaintextEvals) {
        const encrypted = encryptField(evaluation.comments as string);
        evaluation.comments = encrypted;
        await evaluation.save();
    }
    
    console.log(`Migrated ${plaintextEvals.length} evaluations`);
}
```
