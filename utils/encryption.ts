 ...
 */
function encryptDEK(dek: Buffer, masterKey: Buffer): { encrypted: Buffer; iv: Buffer; authTag: Buffer } {
 ...
 */
function decryptDEK(encrypted: Buffer, iv: Buffer, authTag: Buffer, masterKey: Buffer): Buffer {
 ...
 */
export function decryptField(data: EncryptedData): string {
...
export function decryptFields(obj: Record<string, any>, fields: string[]): Record<string, any> {
    ...
    for (const field of fields) {
        if (obj[field] && typeof obj[field] === 'object' && obj[field].encrypted) {
            result[field] = decryptField(obj[field] as EncryptedData);
        }
    }
